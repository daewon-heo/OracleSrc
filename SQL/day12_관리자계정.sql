...
-- 1. SYS / 최상위 관리자
-- 2. SYSTEM / 일반관리자
-- 3. SCOTT (교육용 샘플 계정) / 버전에 따라서 잠겨 있을 수도 있다.
-- 4. HR(샘플계정) / 처음에는 잠겨있으나 버전에 따라 아예 없는 경우도 있다.
--                  11G 버전에서는 별도로 제공하지 않는다.

-- 보안을 위한 데이터베이스 관리자
-- 사용자 계정을 생성하거나, 사용자가 데이터베이스 객체(테이블, 뷰 등) 접근할 수 있는
-- 특정 권한을 역할
-- 다수의 사용자가 공유하는 데이터베이스 객체들, 정보들을 관리하는 역할
-- 따라서 사용자 계정에 따른 권한, 롤(ROLE)을 각각 다르게 부여할 수 있는 계정

-- 권한 : 사용자가 특정 테이블에 접근할 수 있도록 하거나,
--        해당 테이블에 SQL(CRUD) 구문을 사용할 수 있도록 설정하는 일종의 명령어
...
--  BACKUP ANY TABLE : 테이블정보를 백업 할 수 있는 권한
...
-- 사용자가 생성한 테이블, 뷰, 시퀀스 등의 객체들을 실제로
-- 저장하는 물리적인 디스크의 설정들을 관리하는 객체
...
-- 권한을 부여받은 사용자는 자신이 가진 권한에 한해서
-- 다른 사용자에게 자신의 권한을 똑같이 부여해줄 수 있다.

GRANT CREATE SESSION TO EMPLOYEE
WITH ADMIN OPTION;

-- 객체 권한 :
--      테이블이나 뷰, 시퀀스 등 각 객체별로
--      DML(CRUD) 권한을 각각 가질 수 있다.
--      [권한부여 형식]
--      GRANT 권한 종류[(컬럼명)] | ALL 
...
-- WITH GRANT OPTION
-- 사용자가 해당 객체의 권한을 부여 받으면서
-- 해당 권한을 다른 사용자에게도 부여할 수 있는 옵션
...
-- 다수의 사용자가 여러 권한을 필요로 할 때 하나 하나 부여하는 형식이 아닌
-- 특정 역할(ROLE) 단위로 권한을 한꺼번에 부여할 수 있다.
...
--      DNA ROLE
--          사용자가 소유한 데이터베이스 객체들을 관리하고
--          사용자 계정을 생성, 삭제할 수 있는 관리자 권한
