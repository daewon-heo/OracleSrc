...
/*
    ...
    -- 변수의 MODE(유형)
    -- IN : 프로시져에서 사용할 변수 값을 외부에서 잔달 받을 때 사용하는 모드
...
-- 직운 정보를 조회하여 변수에 직원 정보를 삽입한 후 꺼내오기
...
EXEC SAL_EMP_ID(301, :VAR_ENAME, :VAR_SAL, :VAR_JOB);
...
CREATE OR REPLACE PROCEDURE 
    DEL_DEPT_PRINT(D_CODE IN DEPT_01.DEPT_ID%TYPE,
                D_NAME OUT DEPT_01.DEPT_TITLE%TYPE)
IS
BEGIN
    SELECT DEPT_TITLE 
    INTO D_NAME
    ...
    DBMS_OUTPUT.PUT_LINE( D_NAME ||'부서 삭제');
EXCEPTION
    WHEN NO_DATA_FOUND 
    THEN DBMS_OUTPUT.PUT_LINE('해당부서는 없습니다.');
END;
/
-- 다시
...
EXEC DEL_DEPT_PRINT('D2', :DEPT_TITLE);
...
-- 단 매개변수는 사원 번호만을 받으며 연봉 계산식은 하단의 내용과 같다.
-- 연봉 계산식 = (급여 + (급여 * NVL(보너스,0)))*12
--             = (급여 + BONUS_CALC(EMP_ID))*12

CREATE OR REPLACE FUNCTION
    YEAR_SAL_CAL(V_EMP_ID IN EMPLOYEE.EMP_ID%TYPE)
RETURN NUMBER
IS 
    V_SAL EMPLOYEE.SALARY%TYPE;
    V_BONUS EMPLOYEE.BONUS%TYPE;
    CALC_YEAR_SAL NUMBER;
BEGIN
    SELECT SALARY,BONUS_CALC(EMP_ID)
    INTO V_SAL, V_BONUS
    FROM EMPLOYEE
    WHERE EMP_ID = V_EMP_ID;
    
    CALC_YEAR_SAL := (V_SAL +V_BONUS)*12;
    RETURN CALC_YEAR_SAL;
END;
/

VARIABLE RESULT_YEAR_SAL NUMBER;

EXEC :RESULT_YEAR_SAL := YEAR_SAL_CAL('&사번');

SELECT EMP_NAME, TRIM(TO_CHAR(YEAR_SAL_CAL(EMP_ID),'L99,999,999,999')) AS "연봉"
FROM EMPLOYEE;
...
-- 프로시저를 만들어 보시오.
-- 단, 2가지 방식으로 작성하되
-- CURSOR_EMP_01은 1번 방식으로,
-- CURSOR_EMP_02는 3번 방식으로,
...
BEGIN
    ...
    FROM EMPLOYEE
    JOIN DEPARTMENT ON DEPT_CODE = DEPT_ID
    JOIN JOB USING (JOB_CODE)) LOOP
    DBMS_OUTPUT.PUT_LINE('사원명 :' || EMP_INFO.EMP_NAME
                || ', 부서명 : ' || EMP_INFO.DEPT_TITLE
                || ', 직급명 : ' || EMP_INFO.JOB_NAME
                || ', 급여 : ' || EMP_INFO.SALARY);
    END LOOP;
END;
/
...
-- 신입 사원 입사 시에
-- 'OOO 사원이 입사했습니다. 환영해주세요~ ^ㅡ..ㅡ^' 문구 출력하기
...
BEGIN
    ...
    DBMS_OUTPUT.PUT_LINE('환영해주세요~ ^ㅡ..ㅡ^');
END;
/
...
VALUES(SEQ_EID.NEXTVAL, '김강우','961011-1234567', 'Gongil2@naver.com','01012349900', 'D5', 'J7','S5', 3000000,0.1, 200, SYSDATE
,NULL, DEFAULT);
...
INSERT INTO PRODUCT
VALUES(SEQ_PRODUCT.NEXTVAL, '전동퀵보드','샤우미',700000,DEFAULT);
...
INSERT INTO PRODUCT_DETAIL
VALUES(SEQ_DETAIL.NEXTVAL,2, SYSDATE, 900, '입고');

INSERT INTO PRODUCT_DETAIL
VALUES(SEQ_DETAIL.NEXTVAL,3, SYSDATE, 20000, '입고');
...
INSERT INTO PRODUCT_DETAIL
VALUES(SEQ_DETAIL.NEXTVAL,2, SYSDATE, 200, '출고');

INSERT INTO PRODUCT_DETAIL
VALUES(SEQ_DETAIL.NEXTVAL,3, SYSDATE, 3000, '출고');
